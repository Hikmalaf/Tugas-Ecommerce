...
<div class="modal fade" id="modal_form" tabindex="-1" role="dialog" aria-labelledby="modalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form id="form" action="{{ route('update_detail_kuota_tahunan') }}" method="POST">
                @csrf
...
<script>
    ...
    $.ajax({
        ...
        success: function(arrayResponse) {
            ...
            $("#coral_id").val(arrayResponse.data.coral_id);
            $("#quota_amount").val(arrayResponse.data.quota_amount);
            $("#id_edit").val(arrayResponse.data.id);
            $('#modal_form').modal();
            $("#coral_id").change();
        }
    });
